...
docker build --build-arg TAG=${ARLAS_VERSION} -t docker.cloudsmith.io/gisaia/arlas-enterprise/arlas-server:${RELEASE_VERSION} -f ${PROJECT_ROOT_DIRECTORY}/docker/docker-files/Dockerfile-arlas-server .
...
docker push docker.cloudsmith.io/gisaia/arlas-enterprise/arlas-server:${RELEASE_VERSION}
