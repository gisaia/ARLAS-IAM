...
2. Get permission token (RPT) from auth server
...
7. *(Clients/Arlas/Authorization/Resources)* Add any resource `header:name:value` (as name) you need (optionally setting a `type` if you need
...
2. Select the file to import and click `Create` (template is in `conf/realm-export.json`)
